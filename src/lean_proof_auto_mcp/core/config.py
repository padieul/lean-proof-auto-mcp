...
class GrindScoringConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.proof_length_min_for_bonus < 0:
            raise ValueError(f"proof_length_min_for_bonus must be >= 0, got {self.proof_length_min_for_bonus}")
...
class AnnotationValueScoringConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not (-1.0 <= self.low_confidence_penalty <= 0.0):
            raise ValueError(f"low_confidence_penalty must be in [-1.0, 0.0], got {self.low_confidence_penalty}")
...
class ImpactScoringConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0.0 <= self.reusability_score_per_local_lemma <= 1.0):
            raise ValueError(f"reusability_score_per_local_lemma must be in [0.0, 1.0], got {self.reusability_score_per_local_lemma}")
        
        if not (0.0 <= self.reusability_max_score <= 1.0):
            raise ValueError(f"reusability_max_score must be in [0.0, 1.0], got {self.reusability_max_score}")
    ...
    def _from_dict(cls, data: dict) -> "YamlHeuristicsConfig":
        ...
        try:
            ...
            aesop_length_bonuses = [
                ProofLengthBonus(
                    max_lines=bonus_data["max_lines"],
            ...
            aesop_scoring = AesopScoringConfig(
                ...
                structural_tactics_bonus_per_tactic=aesop_data["structural_tactics"]["bonus_per_tactic"],
            ...
            grind_scoring = GrindScoringConfig(
                ...
                algebraic_terms_bonus_per_pattern=grind_data["algebraic_terms"]["bonus_per_pattern"],
                ...
                short_proof_bonus=grind_data["proof_length_bonuses"][1]["bonus"] if len(grind_data["proof_length_bonuses"]) > 1 else 0.08,
                induction_penalty=grind_data["penalties"]["induction"],
                cases_penalty=grind_data["penalties"]["cases"],
                algebraic_tactics_bonus_per_tactic=grind_data["algebraic_tactics"]["bonus_per_tactic"],
            ...
            risk_scoring = RiskScoringConfig(
                global_change_rewrite_heavy_threshold=risk_data["global_change_risk"]["rewrite_heavy"]["threshold"],
                global_change_rewrite_heavy_risk=risk_data["global_change_risk"]["rewrite_heavy"]["risk"],
                global_change_simp_heavy_threshold=risk_data["global_change_risk"]["simp_heavy"]["threshold"],
                global_change_simp_heavy_risk=risk_data["global_change_risk"]["simp_heavy"]["risk"],
                global_change_simp_question_mark_risk=risk_data["global_change_risk"]["simp_question_mark"],
                ...
                local_lemma_moderate_threshold=risk_data["local_lemma_risk"]["moderate"]["threshold"],
                local_lemma_moderate_risk=risk_data["local_lemma_risk"]["moderate"]["risk"],
                low_confidence_very_low_threshold=risk_data["low_confidence_risk"]["very_low"]["threshold"],
            ...
            for score_data in impact_data["proof_length_scores"]:
                impact_length_scores.append(ProofLengthScore(
            ...
            impact_scoring = ImpactScoringConfig(
                ...
                reusability_score_per_local_lemma=impact_data["reusability"]["score_per_local_lemma"],
                ...
                component_weight_annotation_value=impact_data["component_weights"]["annotation_value"],
            ...
            success_likelihood_scoring = SuccessLikelihoodScoringConfig(
                complexity_induction_or_cases_penalty=success_data["complexity_penalties"]["induction_or_cases"],
                complexity_long_proof_threshold=success_data["complexity_penalties"]["long_proof"]["threshold"],
                complexity_long_proof_penalty=success_data["complexity_penalties"]["long_proof"]["penalty"],
                complexity_many_local_lemmas_threshold=success_data["complexity_penalties"]["many_local_lemmas"]["threshold"],
                complexity_many_local_lemmas_penalty=success_data["complexity_penalties"]["many_local_lemmas"]["penalty"],
                ...
                component_weight_complexity_penalty=success_data["component_weights"]["complexity_penalty"],
            )
