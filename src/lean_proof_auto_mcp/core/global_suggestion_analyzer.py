...
class GlobalSuggestionAnalyzer:
    ...
    def _analyze_hint(self, hint: Hint) -> GlobalSuggestion | None:
        ...
        if hint.type == HintType.SIMP:
            ...
            rationale = (
                ...
                f"Adding global @[aesop safe] attribute could improve automation across the project."
            )
    ...
    def _calculate_confidence(
        self,
        hint: Hint,
        category: Literal["simp", "aesop_safe", "unfold"]
        ...
        if category == "simp":
            # Simp lemmas are generally safe to make global
            return "high"
        elif category == "aesop_safe":
            # Safe aesop rules are good candidates
            return "high" if self.config.prefer_safe_rules else "medium"
        elif category == "unfold":
            # Unfold hints are more situational
            return "medium"
        else:
            return "low"
    ...
    def _filter_by_effectiveness(
        self,
        suggestions: list[GlobalSuggestion]
    ) -> list[GlobalSuggestion]:
        """
