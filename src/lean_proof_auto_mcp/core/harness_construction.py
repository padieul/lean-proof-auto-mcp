...
class RegexScopedNotationDetector:
    ...
    def detect(self, theorem_text: str) -> ScopedNotationContext:
        ...
        Examples:
            "open scoped Relator in theorem ..." -> has_scoped_notation=True, scope_modules=["Relator"]
            "open scoped A B C in theorem ..." -> has_scoped_notation=True, scope_modules=["A", "B", "C"]
...
class LeanInteractTheoremTypeExtractor:
    ...
    def extract_type(self, file_path: str, theorem_id: str) -> TheoremType:
        ...
        Args:
            ...
            theorem_id: Fully qualified theorem name (may include namespace like "Subgroup.top_prod_top")
        ...
        for decl in declarations:
            if (decl.name == theorem_id or decl.full_name == theorem_id or
                decl.name == local_name or decl.full_name == local_name):
                theorem_found = True
                break
        ...
        if not full_file_path.exists():
            ...
            for i, part in enumerate(parts):
                if part and part[0].isupper():
                    ...
                    candidate = workspace_path / relative_from_capital if workspace_path else relative_from_capital
                    if candidate.exists():
        ...
        try:
            ...
            return TheoremType(
                type_expr=f"FULL_FILE:{theorem_id}",
                source=f"FullFile:{file_path}"
            )
            
        except Exception as e:
            raise TheoremNotFoundError(
                f"Failed to read source file {file_path}: {e}"
            )
    
    def _infer_free_variables(self, constants: list[str], type_str: str) -> dict[str, str]:
        """
        ...
        for const in constants:
            ...
            if len(const) == 1 and const.isupper() and const in type_str:
                if const not in free_vars:
    ...
    def construct(self, config: HarnessConfig) -> HarnessResult:
        ...
        try:
            ...
            if theorem_type.type_expr.startswith("FULL_FILE:"):
                ...
                workspace_path = self.type_extractor.querier.server_manager.workspace_path
                if workspace_path:
    ...
    def _build_harness_by_proof_replacement(
        ...
        while i < len(lines):
            ...
            if is_theorem or is_lemma:
                ...
                is_target = (decl_name == local_name or 
                           decl_name == theorem_id or
                           stripped.startswith(f'theorem {local_name}') or
                           stripped.startswith(f'lemma {local_name}'))
    ...
    def _skip_proof_block(self, lines: list[str], start_idx: int) -> int:
        ...
        while i < len(lines) and in_proof:
            ...
            if current_indent <= base_indent:
                ...
                if (stripped.startswith('theorem ') or 
                    ...
                    stripped.startswith('/-')):  # Doc comment for next declaration
    ...
    def _validate_harness(self, harness: str) -> None:
        ...
        if not has_import:
            raise HarnessConstructionError(
                "Harness should contain at least one import statement",
        ...
        if not has_theorem:
            raise HarnessConstructionError(
                "Harness must contain at least one theorem or lemma",
                generated_code=harness
            )
