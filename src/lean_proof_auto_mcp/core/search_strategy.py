    ...
    def search(
        ...
        for _step in range(config.max_steps):
            ...
            if best_candidate is not None:
                ...
                if best_candidate_outcome is not None and (
                    best_outcome is None or best_candidate_score > self._score_outcome(best_outcome)
                ):
                    best_hint_set = current_hint_set
                    best_outcome = best_candidate_outcome

                    # Check if we found a closing set
                    if best_outcome.status == "success" and config.stop_on_first_close:
                        return SearchResult(
                            outcome="closed",
                            best_hint_set=best_hint_set,
                            attempts=attempts,
        ...
        final_outcome: Literal["closed", "partial", "failed"]
...
class BeamSearch:
    ...
    def search(
        ...
        beam: list[tuple[HintSet, float, ExecutionOutcome | None]] = [(HintSet(), 0.0, None)]
