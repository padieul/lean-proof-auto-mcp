...
class ProofSkeleton:
    ...
    def add_move(self, move: TacticMove, hint_set: HintSet) -> "ProofSkeleton":
        ...
        return ProofSkeleton(
            ...
            hint_sets=self.hint_sets + (hint_set,),
            depth=self.depth + 1
        )
...
class SkeletonExplorer:
    ...
    def explore(
        ...
        logger.info(f"Starting skeleton exploration (max_depth={config.max_depth}, moves={config.moves})")
        ...
        if best_skeleton is not None:
            logger.info(f"No closing skeleton found, best partial result at depth {best_skeleton.depth}")
            return SkeletonSearchResult(
                outcome="partial",
            ...
            logger.info(f"No skeletons explored")
            return SkeletonSearchResult(
                outcome="failed",
                best_skeleton=None,
                attempts=attempts,
    ...
    def _is_better_outcome(
        self,
        new_evidence: ExecutionOutcome,
        current_best: ExecutionOutcome | None
    ) -> bool:
        """
