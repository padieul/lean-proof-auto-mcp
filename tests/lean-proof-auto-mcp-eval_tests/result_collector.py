...
class ResultCollector:
    ...
    def summary(self) -> dict[str, Any]:
        ...
        for tool, results in groupby(sorted(self._results, key=lambda r: r.tool), key=lambda r: r.tool):
        ...
        for domain, results in groupby(sorted(self._results, key=lambda r: r.domain), key=lambda r: r.domain):
            results_list = list(results)
            status_counts = Counter(r.status for r in results_list)
            by_domain[domain] = {
                'success': status_counts.get('success', 0),
    ...
    def compare_baseline(self, baseline_path: Path) -> dict[str, Any]:
        ...
        for key, current_result in current_map.items():
            if key in baseline_map:
                ...
                if baseline_status == 'success' and current_status in ['failure', 'error', 'timeout']:
                    regressions.append({
                ...
                elif baseline_status in ['failure', 'error', 'timeout'] and current_status == 'success':
                    improvements.append({
                        'tool': current_result.tool,
                        'file_path': current_result.file_path,
                        'theorem_id': current_result.theorem_id,
