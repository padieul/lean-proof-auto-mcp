...
CONTEXT_THEOREMS: list[tuple[str, str]] = [
    ...
    (r"Algebra\Polynomial\FieldDivision.lean", "Polynomial.isRoot_iterate_derivative_of_lt_rootMultiplicity"),
    ...
    (r"LinearAlgebra\LinearIndependent\Defs.lean", "linearIndependent_iff_injective_finsuppLinearCombination"),
    ...
    (r"RingTheory\Ideal\Quotient\ChineseRemainder.lean", "Ideal.pi_tensorProductMk_quotient_surjective"),
...
def _assert_get_proof_context_response_structure(response: dict[str, Any]) -> None:
    ...
    if not passed:
        raise AssertionError(
            f"get_proof_context response structure invalid: {'; '.join(failures)}"
        )
