...
class TestProperty10ArtifactStorageCompleteness:
            ...
            diagnostics=[
                {
                    ...
                    "location": {"file": "test.lean", "line": 10, "col": 5, "end_line": None, "end_col": None},
                }
            ],
            diagnostic_summary={"error_count": 1, "warning_count": 0, "info_count": 0},
            evidence={"stdout_excerpt": "Error output", "stderr_excerpt": "", "notes": ["exit_code: 1"]},
    ...
    @given(
        file_path=st.text(min_size=1, max_size=100, alphabet=st.characters(blacklist_categories=("Cs",))),
    ...
    def test_artifact_store_handles_various_inputs(self, file_path, budget_s):
        ...
        try:
            ...
            with open(run_dir / "request.json", "r", encoding="utf-8") as f:
                request_data = json.load(f)
...
class TestProperty19ArtifactPersistence:
    ...
    def test_artifacts_persist_after_store_returns(self):
        ...
        try:
            ...
            with open(run_dir / "lean_output.log", "r", encoding="utf-8") as f:
                log_content = f.read()
            assert log_content == full_logs
