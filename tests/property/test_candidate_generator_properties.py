...
def valid_theorem_decl(draw):
    ...
    theorem_id = draw(st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'), whitelist_characters='._')))
...
"""
    ...
    extracted_sources = {candidate.hint.source for candidate in candidates}
    ...
    for source_type in config.sources:
        ...
        if source_candidates:
            ...
            source_names = {c.hint.name for c in source_candidates}
            final_names = {c.hint.name for c in candidates}
...
"""
    ...
    for source_type in selected_sources:
        ...
        assert len(source_candidates) <= max_candidates, \
            f"Source {source_type} has {len(source_candidates)} candidates, exceeds limit {max_candidates}"
...
"""
    ...
    sources = [CandidateSource.SAME_NAMESPACE, CandidateSource.NEARBY_DECLS, CandidateSource.ORIGINAL_PROOF_REFS]
...
"""
    ...
    non_def_candidates = [c for c in candidates if '_def' not in c.hint.name and '.def' not in c.hint.name]
    ...
    if def_candidates and non_def_candidates:
        ...
        assert min_def_rank >= max_non_def_rank - 1.0, \
            f"Def lemmas should have higher priority: min_def={min_def_rank}, max_non_def={max_non_def_rank}"
...
"""
    ...
    proof_ref_candidates = [c for c in candidates if c.hint.source == CandidateSource.ORIGINAL_PROOF_REFS]
    other_candidates = [c for c in candidates if c.hint.source != CandidateSource.ORIGINAL_PROOF_REFS]
