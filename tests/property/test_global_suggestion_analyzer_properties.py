...
def test_property_29_mode_conditional_global_suggestions(hint_set, config):
    ...
    if hint_set.is_empty():
        assert global_suggestions == [], (
            "Empty hint set should produce no suggestions"
        )
    else:
        # Suggestions should be a list (may be empty if no hints are suitable)
        assert isinstance(global_suggestions, list), (
            "suggest_global mode should return a list"
        )
        ...
        for suggestion in global_suggestions:
            assert hasattr(suggestion, 'hint_name'), (
                "Each suggestion should have hint_name"
            )
            assert hasattr(suggestion, 'attribute'), (
                "Each suggestion should have attribute"
            )
            assert hasattr(suggestion, 'rationale'), (
                "Each suggestion should have rationale"
            )
...
def test_property_30_effective_hint_suggestion_generation(hint_set, config):
    ...
    hints_that_should_suggest = [
        hint for hint in hint_set.hints
        if _should_generate_suggestion(hint, config)
    ]
