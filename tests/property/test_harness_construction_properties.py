...
def valid_theorem_ids(draw):
    ...
    parts = draw(st.lists(st.text(min_size=1, max_size=20, alphabet=st.characters(
...
def valid_file_paths(draw):
    ...
    for i in range(num_parts):
        ...
        rest = draw(st.text(min_size=0, max_size=15, alphabet=st.characters(
            whitelist_categories=("Lu", "Ll", "Nd"),
            whitelist_characters="_"
        )))
    ...
    for i, line in enumerate(lines):
        if line.strip():
            if line.strip().startswith("import"):
                assert not found_non_import, \
                    f"Import at line {i+1} appears after non-import code"
            else:
...
def test_property_2_no_signature_reconstruction(config, theorem_type):
    ...
    **Validates: Requirements 1.4**
    ...
    assert "example :" in result.code, \
        "Harness must use 'example :' statement"
...
def test_property_3_type_preservation(config, theorem_type):
    ...
    **Validates: Requirements 1.2**
    ...
    assert example_index < type_index, \
        "Theorem type must appear after 'example :'"
...
@settings(max_examples=100)
def test_property_deterministic_construction(
    theorem_id, file_path, proof_attempt, theorem_type
):
    """
    Feature: import-based-theorem-testing
    ...
    **Validates: Requirements 1.1**
    ...
    assert result1.code == result2.code, \
        "Harness construction must be deterministic"
...
def test_property_error_handling_theorem_not_found(config):
    ...
    **Validates: Requirements 1.2**
    ...
    assert isinstance(result, HarnessError), \
        "Result must be HarnessError when theorem not found"
    
    # Verify: Error type is correct
    assert result.error_type == "theorem_not_found", \
        "Error type must be 'theorem_not_found'"
    
    # Verify: Error contains theorem ID
    assert config.theorem_id in result.message, \
        "Error message must contain theorem ID"
...
def test_property_metadata_preservation(config, theorem_type):
    ...
    **Validates: Requirements 1.1**
    ...
    assert result.theorem_id == config.theorem_id, \
        "Theorem ID must be preserved in result"
    assert result.file_path == config.file_path, \
        "File path must be preserved in result"
