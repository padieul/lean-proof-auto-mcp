...
def scan_file_validator() -> Draft202012Validator:
    ...
    registry = Registry().with_resources([
        ("common.json", Resource.from_contents(common_schema, default_specification=DRAFT202012)),
        ("scan_file.json", Resource.from_contents(scan_schema, default_specification=DRAFT202012)),
    ])
...
def lean_like_content(draw) -> str:
    ...
    # Optional namespace
    if draw(st.booleans()):
        namespace = draw(st.text(alphabet=st.characters(whitelist_categories=("Lu", "Ll")), min_size=1, max_size=10))
    ...
    for i in range(num_theorems):
        ...
        theorem_name = draw(st.text(alphabet=st.characters(whitelist_categories=("Lu", "Ll")), min_size=1, max_size=15))
        ...
        tactics = draw(st.lists(
            st.sampled_from(["rfl", "simp", "intro", "cases n", "induction n", "rw [h]", "exact h"]),
...
class TestScanFileProperties:
    ...
    @given(args=scan_file_args())
    def test_schema_compliance(self, args: dict[str, Any], scan_file_validator: Draft202012Validator) -> None:
        ...
        **Validates: Requirements 1.7**
        ...
        if "theorems" in result:
            for theorem in result["theorems"]:
                ...
                assert 0.0 <= wgp["aesop"] <= 1.0, f"aesop whole_goal_potential out of bounds: {wgp['aesop']}"
                assert 0.0 <= wgp["grind"] <= 1.0, f"grind whole_goal_potential out of bounds: {wgp['grind']}"
                
                # Check subgoal_potential scores  
                sgp = auto["subgoal_potential"]
                assert 0.0 <= sgp["aesop"] <= 1.0, f"aesop subgoal_potential out of bounds: {sgp['aesop']}"
                assert 0.0 <= sgp["grind"] <= 1.0, f"grind subgoal_potential out of bounds: {sgp['grind']}"
    ...
    def test_stable_ordering(self, args: dict[str, Any]) -> None:
        ...
        **Validates: Requirements 1.11**
        ...
        if "theorems" in result and len(result["theorems"]) > 1:
            ...
            assert theorem_ids == sorted_ids, f"Theorems not in stable order: {theorem_ids} != {sorted_ids}"
        ...
        if "theorems" in result:
            for theorem in result["theorems"]:
                ...
                if "proof_start" in loc and "proof_end" in loc:
                    assert loc["proof_start"] >= 1, f"proof_start must be >= 1, got {loc['proof_start']}"
                    assert loc["proof_end"] >= 1, f"proof_end must be >= 1, got {loc['proof_end']}"
                    assert loc["proof_start"] <= loc["proof_end"], (
                        f"proof_start ({loc['proof_start']}) must be <= proof_end ({loc['proof_end']})"
                    )
                    
                    # Proof should be within or after declaration
                    assert loc["proof_start"] >= loc["decl_start"], (
                        f"proof_start ({loc['proof_start']}) must be >= decl_start ({loc['decl_start']})"
                    )
            ...
            for theorem in result["theorems"]:
                if "notes" in theorem:
                    for note in theorem["notes"]:
                        assert isinstance(note, str), f"Theorem note must be string, got {type(note)}: {note}"