...
def test_property_38_timing_completeness(result):
    ...
    if result.status != "error" or result.viability.get("status") == "success":
        ...
        assert isinstance(timing["viability_check_s"], (int, float)), "viability_check_s must be numeric"
    ...
    if result.baseline.get("status") in ("completed", "timeout", "error"):
        if "baseline_probe_s" in timing:
            assert isinstance(timing["baseline_probe_s"], (int, float)), "baseline_probe_s must be numeric"
    ...
    if result.minimized_hint_set is not None and result.search_result is not None:
        if "minimize_total_s" in timing:
            assert isinstance(timing["minimize_total_s"], (int, float)), "minimize_total_s must be numeric"
    ...
    if result.proof_patch is not None:
        if "proof_patch_build_s" in timing:
            assert isinstance(timing["proof_patch_build_s"], (int, float)), "proof_patch_build_s must be numeric"
...
def test_property_39_result_structure_completeness(result):
    ...
    if result.status == "success":
        assert result.minimized_hint_set is not None, \
            "Success status requires minimized_hint_set"
    ...
    if result.status == "success":
        assert result.proof_patch is not None, \
            "Success status requires proof_patch"
        assert result.proof_patch.lean_code, \
            "Proof patch must have non-empty lean_code"
    
    # Requirement 10.6: Global annotation suggestions (if mode is suggest_global)
    assert "global_suggestions" in result_dict, \
        "Result must include global_suggestions field"
...
def test_property_40_result_determinism(result):
    ...
    assert json_str1 == json_str2, \
        "Multiple JSON serializations of same result must be identical"
    ...
    if result.minimized_hint_set is not None:
        ...
        assert len(hints_list1) == len(hints_list2), \
            "Sorted hint lists must have same length"
        
        for h1, h2 in zip(hints_list1, hints_list2):
    ...
    if result.proof_patch is not None:
        ...
        assert patch1.strip() == patch2.strip(), \
            "Proof patch whitespace must be consistent"
    ...
    assert json_str1 == json_str3, \
        "JSON must remain stable after parse/serialize cycle"
