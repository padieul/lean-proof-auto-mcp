...
def test_property_2_error_descriptiveness(cmd):
    ...
    if result.status in ("fail", "error", "timeout"):
        ...
        has_error_info = (
            "error" in result.viability or
            "error" in result.baseline or
            "error" in result.metadata or
            result.viability.get("status") in ("error", "not_started") or
            result.baseline.get("status") in ("error", "not_started")
        )
...
def test_property_3_budget_enforcement(viability_budget, baseline_budget):
    ...
    cmd = SearchAnnotationsCommand(
        ...
        automation=AutomationConfig(),
        budgets=BudgetConfig(
            viability_check_s=viability_budget,
            baseline_probe_s=baseline_budget
        ),
        search=SearchConfig(),
        candidates=CandidateConfig(sources=[CandidateSource.GOAL_SYMBOLS]),
        skeleton=SkeletonConfig(),
...
def test_property_7_early_termination_on_baseline_success(cmd):
    ...
    from lean_proof_auto_mcp.core.probe_domain import ProbeResult, ProbeOutcome
    ...
    if result.baseline.get("primary_outcome") == "closed":
        ...
        assert not candidate_generator.generate.called, "Candidates should not be generated when baseline succeeds"
        
        # Search strategy should not have been called
        assert not search_strategy.search.called, "Search should not be performed when baseline succeeds"
