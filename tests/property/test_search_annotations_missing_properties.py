...
from lean_proof_auto_mcp.core.indexer import TheoremDecl, build_index
...
def test_property_5_automation_configuration_propagation(config):
    ...
    if config.aesop_rules is not None:
        assert isinstance(config.aesop_rules, dict), \
            "Aesop rules should be a dict when present"
    else:
        # None is also valid
        assert config.aesop_rules is None, \
            "Aesop rules should be None or dict"
...
def test_property_6_fallback_automation_attempt(primary, secondary):
    ...
    assert config.primary == primary, \
        f"Primary should be {primary}, got {config.primary}"
    assert config.secondary == secondary, \
        f"Secondary should be {secondary}, got {config.secondary}"
...
@given(
    allow_simp=st.booleans(),
    allow_unfold=st.booleans()
)
@settings(max_examples=20)
def test_property_10_conditional_hint_type_inclusion(allow_simp, allow_unfold):
    ...
    assert config.allow_simp_hints == allow_simp, \
        f"allow_simp_hints should be {allow_simp}"
    assert config.allow_unfold_hints == allow_unfold, \
        f"allow_unfold_hints should be {allow_unfold}"
    ...
    if index.decls:
        ...
        if not allow_simp:
            assert len(simp_candidates) == 0, \
                f"SIMP hints should be excluded when allow_simp_hints=False, found {len(simp_candidates)}"
        
        # Verify: UNFOLD hints included/excluded based on flag
        if not allow_unfold:
            assert len(unfold_candidates) == 0, \
                f"UNFOLD hints should be excluded when allow_unfold_hints=False, found {len(unfold_candidates)}"
...
def test_property_15_success_recording(num_candidates, success_after):
    ...
    if success_after <= num_candidates:
        assert result is not None, \
            "Successful hint set should be recorded"
        assert result.best_hint_set is not None, \
            "Result should have best_hint_set"
        ...
        assert outcome.status == "success", \
            "Recorded hint set should successfully close the goal"
    ...
    if simp_hints:
        ...
        if simp_only:
            assert "simp only" in patch.lean_code or "simp_all only" in patch.lean_code, \
                f"When simp_only_list=True with SIMP hints, should use 'simp only' or 'simp_all only', got: {patch.lean_code}"
        else:
            ...
            assert "simp_all only" in patch.lean_code or "simp only" in patch.lean_code, \
                f"When simp_only_list=False with SIMP hints, should use 'simp_all only' or 'simp only', got: {patch.lean_code}"
    else:
...
def test_property_28_proof_patch_completeness(hint_set, automation, style):
    ...
    if not hint_set.is_empty():
        ...
        assert (automation in patch.lean_code.lower() or 
                patch.lean_code.strip() in ["rfl", "trivial", "Iff.rfl", "simp", "aesop", "grind"]), \
            f"Proof patch should mention automation tool '{automation}' or be a valid tactic, got: {patch.lean_code}"
    ...
    if not hint_set.is_empty():
        ...
        assert len(patch.lean_code) > 0, \
            "Proof patch should be non-empty for non-empty hint set"
    
    # Verify: Patch is valid Lean syntax structure (relaxed check)
    # Accept various valid Lean proof formats
    is_valid = (
        patch.lean_code.strip() in ["rfl", "trivial", "Iff.rfl", "simp", "aesop", "grind"] or
        "by" in patch.lean_code or
        "[" in patch.lean_code or
        "(" in patch.lean_code  # aesop with hints uses parentheses
    )
    assert is_valid, \
        f"Proof patch should be valid Lean proof syntax, got: {patch.lean_code}"
    
    # Verify: Patch metadata is complete
    assert patch.hint_set == hint_set, \
        "Proof patch should preserve hint set"
    assert patch.automation == automation, \
        "Proof patch should preserve automation tool"
    assert patch.style == style, \
        "Proof patch should preserve style config"
...
def test_property_31_suggestion_rationale_completeness(hint_set):
    ...
    config = AnalysisConfig(
        ...
        prefer_safe_rules=True,
        suggest_simp_for_equations=True
    )
    ...
    for suggestion in suggestions:
        assert hasattr(suggestion, 'rationale'), \
            "Each suggestion should have a rationale field"
        ...
        assert suggestion.hint_name in suggestion.rationale or \
               any(word in suggestion.rationale.lower() for word in ["hint", "lemma", "rule", "effective"]), \
...
def test_property_32_global_edit_safety(hint_set, allow_edits):
    ...
    for suggestion in suggestions:
        assert hasattr(suggestion, 'attribute'), \
            "Suggestion should have attribute field"
        assert hasattr(suggestion, 'hint_name'), \
            "Suggestion should have hint_name field"
...
def test_property_34_worktree_cleanup_control(keep_artifacts):
    ...
    assert config.keep_artifacts == keep_artifacts, \
        f"keep_artifacts should be {keep_artifacts}"
    
    # Verify: Mode is set correctly
    assert config.mode == "git_worktree", \
        "Workspace mode should be git_worktree"
...
def test_property_44_style_preference_application(hint_set, prefer_simp):
    ...
    assert ("simp" in simp_patch.lean_code.lower() or 
            simp_patch.lean_code.strip() in ["rfl", "trivial", "Iff.rfl"]), \
    ...
    if prefer_simp:
        ...
        assert ("simp" in aesop_patch.lean_code.lower() or 
                "aesop" in aesop_patch.lean_code.lower() or
                aesop_patch.lean_code.strip() in ["rfl", "trivial", "Iff.rfl"]), \
            f"Aesop patch with prefer_simp=True should use simp or aesop, got: {aesop_patch.lean_code}"
    else:
        # When prefer_simp is False, should use aesop (or be definitional)
        assert ("aesop" in aesop_patch.lean_code.lower() or 
                aesop_patch.lean_code.strip() in ["rfl", "trivial", "Iff.rfl"]), \
            f"Aesop patch with prefer_simp=False should use aesop or be definitional, got: {aesop_patch.lean_code}"
