...
def valid_hints(draw):
    ...
    name = draw(
        st.text(
            min_size=1, max_size=100, alphabet=st.characters(min_codepoint=65, max_codepoint=122)
        )
    )
...
@given(
    candidates=st.lists(
        valid_candidates(), min_size=1, max_size=20, unique_by=lambda c: c.hint.name
    ),
...
def test_property_13_greedy_search_incremental_growth(candidates, config):
    ...
    strategy.search(candidates=candidates, probe_fn=tracking_probe, config=config, budget_s=10.0)
    ...
    if observed_sizes:
        ...
        assert observed_sizes[0] == 1, (
            f"First probed set should have size 1, got {observed_sizes[0]}"
        )
...
@given(
    candidates=st.lists(
        valid_candidates(), min_size=5, max_size=10, unique_by=lambda c: c.hint.name
    ),
...
@given(
    candidates=st.lists(
        valid_candidates(), min_size=3, max_size=10, unique_by=lambda c: c.hint.name
    ),
...
def test_property_13_greedy_search_stops_at_success(candidates, success_after):
    ...
    """
    config = SearchConfig(
        strategy="greedy", beam_width=3, max_steps=100, max_hints=20, stop_on_first_close=True
    )
    ...
    def tracking_probe(hint_set: HintSet) -> ExecutionOutcome:
        ...
        if hint_set.size() >= success_after:
            ...
            outcome = ExecutionOutcome(
                status="failure", automation_used="aesop", duration_s=0.1, output="Failed"
            )
    ...
    result = strategy.search(
        candidates=candidates, probe_fn=tracking_probe, config=config, budget_s=10.0
    )
...
@given(
    candidates=st.lists(
        valid_candidates(), min_size=5, max_size=20, unique_by=lambda c: c.hint.name
    ),
...
@given(
    candidates=st.lists(
        valid_candidates(), min_size=5, max_size=15, unique_by=lambda c: c.hint.name
    ),
...
@given(
    candidates=st.lists(
        valid_candidates(), min_size=10, max_size=30, unique_by=lambda c: c.hint.name
    ),
...
def test_property_17_hint_set_size_limit(candidates, max_hints, strategy_type):
    ...
    # Execute: Search
    strategy = GreedySearch() if strategy_type == "greedy" else BeamSearch()
...
@settings(max_examples=100)
def test_property_16_search_termination_control(
    candidates, success_after, stop_on_first_close, strategy_type
):
    """
    Feature: search-annotations-tool
    Property 16: Search Termination Control
    ...
    if found_success_at_attempt is not None and stop_on_first_close:
        ...
        assert total_attempts <= max_expected_attempts, (
            f"With stop_on_first_close=True, expected <= {max_expected_attempts} "
            f"attempts, got {total_attempts}"
        )
...
def test_property_18_hint_set_scoring_consistency(candidates, strategy_type):
    ...
    """
    config = SearchConfig(
        strategy=strategy_type, beam_width=3, max_steps=10, max_hints=10, stop_on_first_close=False
    )
    ...
    def varied_outcome_probe(hint_set: HintSet) -> ExecutionOutcome:
        ...
        return ExecutionOutcome(
            status=status, automation_used="aesop", duration_s=0.1, output=f"Status: {status}"
        )
    ...
    result = strategy.search(
        candidates=candidates, probe_fn=varied_outcome_probe, config=config, budget_s=10.0
    )

    # Verify: Best outcome should be success if we reached size 5
    if result.best_hint_set and result.best_hint_set.size() >= 5:
        assert result.outcome == "closed", "Search should have found a closing set"
        assert result.evidence is not None, "Closing set should have evidence"
...
def test_property_18_scoring_function_ordering(strategy_type):
    ...
    success_outcome = ExecutionOutcome(
        status="success", automation_used="aesop", duration_s=0.1, output="Success"
    )

    failure_outcome = ExecutionOutcome(
        status="failure", automation_used="aesop", duration_s=0.1, output="Failure"
    )

    timeout_outcome = ExecutionOutcome(
        status="timeout", automation_used="aesop", duration_s=0.1, output="Timeout"
    )

    error_outcome = ExecutionOutcome(
        status="error", automation_used="aesop", duration_s=0.1, output="Error", error="Some error"
    )
