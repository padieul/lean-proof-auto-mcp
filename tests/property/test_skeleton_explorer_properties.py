...
def test_property_41_skeleton_mode_conditional_behavior_disabled(config):
    ...
    assert result.outcome == "failed", \
        "Disabled skeleton search should return failed outcome"
    assert result.best_skeleton is None, \
        "Disabled skeleton search should not return a skeleton"
    assert result.attempts == 0, \
        "Disabled skeleton search should make no attempts"
    assert result.explored_skeletons == 0, \
        "Disabled skeleton search should explore no skeletons"


@given(
    max_depth=st.integers(min_value=1, max_value=5),
    moves=st.lists(
        st.sampled_from(["cases", "constructor", "induction"]),
        min_size=1,
        max_size=3,
        unique=True
    )
...
def test_property_41_skeleton_mode_conditional_behavior_enabled(max_depth, moves):
    ...
    assert result.attempts > 0, \
        "Enabled skeleton search should make at least one attempt"
...
def test_property_41_skeleton_mode_finds_solution_at_depth(max_depth, target_depth):
    ...
    config = SkeletonConfig(
        enabled=True,
        max_depth=max_depth,
        moves=["cases", "constructor", "induction"]
    )
...
def test_property_42_skeleton_depth_limit_all_skeletons(max_depth, moves):
    ...
    for depth in all_depths:
        assert depth <= max_depth, \
            f"Found skeleton with depth {depth}, max_depth is {max_depth}"
...
def test_property_42_skeleton_depth_limit_stops_at_limit(max_depth, success_depth):
    ...
    assert result.outcome != "closed", \
        f"Should not find closing skeleton (success at depth {success_depth}, max_depth {max_depth})"
...
def test_skeleton_explorer_respects_budget(max_depth, moves, budget_s):
    ...
    assert elapsed <= budget_s + 1.0, \
        f"Exploration took {elapsed}s, budget was {budget_s}s"
...
def test_skeleton_explorer_combines_with_hint_search(max_depth, target_depth):
    ...
    assert len(unique_depths) > 0, \
        "Hint search should be called for at least one depth"
