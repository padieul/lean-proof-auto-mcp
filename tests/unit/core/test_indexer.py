...
class TestTheoremDecl:
    ...
    def test_theorem_decl_creation_valid(self):
        ...
        decl = TheoremDecl(
            theorem_id="Nat.add_comm", name="add_comm", kind="theorem", decl_span=span
        )
    ...
    def test_theorem_decl_invalid_theorem_id(self):
        ...
        with pytest.raises(ValueError, match="theorem_id cannot be empty"):
            TheoremDecl(theorem_id="", name="test", kind="theorem", decl_span=span)

    def test_theorem_decl_invalid_name(self):
        """Test that name cannot be empty."""
        span = Span(start_line=1, end_line=1)
        with pytest.raises(ValueError, match="name cannot be empty"):
            TheoremDecl(theorem_id="test", name="", kind="theorem", decl_span=span)

    def test_theorem_decl_invalid_kind(self):
        """Test that kind must be valid."""
        span = Span(start_line=1, end_line=1)
        with pytest.raises(ValueError, match="Invalid kind"):
            TheoremDecl(theorem_id="test", name="test", kind="invalid", decl_span=span)
    ...
    def test_theorem_decl_valid_kinds(self):
        ...
        for kind in valid_kinds:
            decl = TheoremDecl(theorem_id="test", name="test", kind=kind, decl_span=span)
...
instance : Inhabited Nat := ⟨0⟩"""
        ...
        assert instance_decl.theorem_id.startswith(
            "instance_"
        )  # Generated name for unnamed instance
...
"""
        ...
        assert len(theorem_ids) == len(set(theorem_ids)), (
            f"Duplicate theorem_id values found: {theorem_ids}"
        )
        ...
        assert (
            len(namespaced_decls) >= 3
        )  # 3 declarations in Test namespace (example, instance, theorem)
        ...
        for decl in anonymous_decls:
            if "Test." in decl.theorem_id:
                ...
                assert decl.theorem_id.startswith(expected_pattern), (
                    f"Expected {decl.theorem_id} to start with {expected_pattern}"
                )
            else:
            ...
            if suffix.isdigit():
                ...
                if len(parts) >= 3 and parts[-1].isdigit() and parts[-2].isdigit():
                    ...
                    counter = int(parts[-1])
                    assert line_num > 0 and counter > 0, (
                        f"Line number and counter should be positive: {decl.theorem_id}"
                    )
...
class TestFindByRange:
    ...
    def sample_index(self):
        ...
        decls = [
            ...
            TheoremDecl(
                "third", "third", "theorem", Span(start_line=10, end_line=15)
            ),  # lines 10-15
        ]
        return FileIndex(file="test.lean", decls=decls)
...
theorem A.B.C.D.E.deeply_nested : True := rfl
    ...
    def test_overlapping_ranges(self):
        ...
        # Create index with overlapping declarations (shouldn't happen in real Lean, but test)
        decls = [
