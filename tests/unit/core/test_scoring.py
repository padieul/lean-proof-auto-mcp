...
class TestAutomationProfile:
    ...
    def test_invalid_whole_goal_score_low(self):
        ...
        with pytest.raises(ValueError, match="whole_goal_potential\\[aesop\\] must be in \\[0.0, 1.0\\]"):
            AutomationProfile(
    ...
    def test_invalid_whole_goal_score_high(self):
        ...
        with pytest.raises(ValueError, match="whole_goal_potential\\[grind\\] must be in \\[0.0, 1.0\\]"):
            AutomationProfile(
    ...
    def test_invalid_subgoal_score(self):
        ...
        with pytest.raises(ValueError, match="subgoal_potential\\[aesop\\] must be in \\[0.0, 1.0\\]"):
            AutomationProfile(
                whole_goal_potential={"aesop": 0.8, "grind": 0.3},
...
class TestComputeProfile:
    ...
    def test_profile_with_structure(self):
        ...
        structure = ProofStructure(
            ...
            cases=[
                CaseBlock("zero", Span(2, 0, 4, 0)),
                CaseBlock("succ", Span(6, 0, 14, 0))
            ]
        )
...
class TestIntegrationScenarios:
    ...
    def test_mathlib_induction_proof(self):
        ...
        structure = ProofStructure(
            ...
            cases=[
                CaseBlock("zero", Span(2, 0, 5, 0)),
                CaseBlock("succ", Span(6, 0, 11, 0))
            ]
        )
    ...
    def test_complex_mixed_proof(self):
        ...
        structure = ProofStructure(
            ...
            cases=[
                CaseBlock("base", Span(5, 0, 12, 0)),
                CaseBlock("step", Span(13, 0, 30, 0))
            ]
        )
...
class TestBoundaryConditions:
    ...
    def test_rounding_precision(self):
        ...
        decimal_part = str(profile.annotation_value).split('.')[-1] if '.' in str(profile.annotation_value) else ""
        assert len(decimal_part) <= 2